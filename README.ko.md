# ppopcode

**Claude 기반 터미널 AI 코딩 비서**

---

### 왜 ppopcode인가요?

**"브라우저에서 AI 채팅 열고, IDE 열고... 너무 번거로워요"**

이런 경험 있으시죠? ppopcode는 Claude를 터미널로 가져와서 이 문제를 해결합니다.

---

### 핵심 기능

| 기능 | 설명 |
|------|------|
| **터미널 UI** | 깔끔하고 효율적인 TUI 인터페이스 |
| **Claude 기반** | Claude Code로 지능적인 응답 |
| **Cursor 연동** | 코드 수정은 Cursor가 자동 처리 |
| **워크플로우** | 반복 작업을 자동화 |
| **세션 컨텍스트** | 대화 기록 유지 |

---

### 이런 분께 추천해요

- 터미널 기반 작업 환경을 선호하시는 분
- Cursor 구독료를 제대로 활용하고 싶은 분
- 터미널을 떠나지 않고 AI 코딩 도움을 받고 싶은 분

---

### Before vs After

**Before (기존 방식)**
```
1. 브라우저에서 AI 채팅 열기
2. 질문하고 답변 복사
3. Cursor 열기
4. 붙여넣기
5. 막히면 다시 1번으로...
→ 반복, 반복, 반복
```

**After (ppopcode)**
```
1. 터미널에서 ppopcode 실행
2. 원하는 것 입력
3. 끝!
```

---

## 설치 방법

### 1단계: Go 설치하기

Go가 없으면 먼저 설치해야 해요.

**Windows:**
1. https://go.dev/dl/ 에서 Windows용 설치파일 다운로드
2. 설치파일 실행
3. 터미널(PowerShell) 재시작

**Mac:**
```bash
brew install go
```

**Linux:**
```bash
sudo apt install golang-go
```

설치 확인:
```bash
go version
```
버전이 나오면 성공!

### 2단계: 전역 설치 (권장)

한 번 설치하면 어디서든 사용할 수 있어요!

**Linux/Mac:**
```bash
git clone https://github.com/ppopcode/ppopcode.git
cd ppopcode
make install
```

**Windows (PowerShell):**
```powershell
git clone https://github.com/ppopcode/ppopcode.git
cd ppopcode
make install
# 또는: .\scripts\install.ps1
```

설치 완료! 이제 어디서든 `ppopcode` 명령어로 실행할 수 있어요.

### 실행하기

**전역 설치 후 (권장):**
```bash
# 어느 폴더에서든 실행!
ppopcode
```

**개발용 로컬 실행:**
```bash
# 빌드하고 바로 실행
make run

# 또는 직접 실행
./ppopcode        # Linux/Mac
.\ppopcode.exe    # Windows
```

### 제거하기

```bash
# Linux/Mac
make uninstall
# 또는: ./scripts/install.sh uninstall

# Windows (PowerShell)
.\scripts\install.ps1 -Uninstall
```

## cc-wf-studio 설치 (선택사항)

워크플로우 기능을 사용하려면 cc-wf-studio VSCode 익스텐션이 필요해요.

### 설치 방법

1. **VSCode 또는 Cursor 열기**

2. **익스텐션 탭 열기**
   - 왼쪽 사이드바에서 네모 4개 아이콘 클릭
   - 또는 `Ctrl+Shift+X` (Windows) / `Cmd+Shift+X` (Mac)

3. **검색창에 입력**
   ```
   cc-wf-studio
   ```

4. **Install 버튼 클릭**

### 사용법

1. VSCode/Cursor에서 `Ctrl+Shift+P` (또는 `Cmd+Shift+P`)
2. `cc-wf-studio` 입력
3. 워크플로우 에디터가 열려요
4. 노드를 드래그해서 작업 흐름을 만들어요
5. 저장하면 `.vscode/workflows/` 폴더에 JSON 파일이 생겨요

### 워크플로우란?

워크플로우는 "작업 순서"를 미리 정해놓는 거예요.

예를 들어 "새 프로젝트 시작" 워크플로우를 만들면:
1. 아이디어 문서 읽기
2. 계획 세우기
3. 코드 작성하기

이 순서를 매번 말할 필요 없이, 워크플로우 하나만 선택하면 자동으로 진행돼요!

## 인증 설정

**ppopcode의 "Link Accounts" 메뉴에서 인증 상태를 확인하고 설정할 수 있어요!**

### Claude (필수)

Claude Code 구독이 필요해요. API 키가 아닌 구독 계정으로 로그인해요.

```bash
claude login
```

터미널에서 위 명령어를 실행하면 브라우저가 열리고 로그인할 수 있어요.

### Cursor (선택사항, 코드 수정 기능용)

Cursor IDE를 열고 구독 계정으로 로그인하세요.

## 사용법

### 메인 메뉴

프로그램을 실행하면 메뉴가 나와요:

- **Link Accounts**: Claude와 Cursor 인증 상태 확인 및 설정
- **Chat**: AI와 대화하면서 코딩하기
- **Workflow**: 미리 만든 작업 흐름 실행하기
- **Settings**: 설정 바꾸기

### 키보드 조작

- `↑` `↓` 또는 `j` `k`: 위/아래 이동
- `Enter`: 선택
- `Esc`: 뒤로가기
- `q`: 종료
- `/clean`: 채팅 기록 지우기

### 예시

```
Chat 선택 → "로그인 폼 만들어줘" 입력 → Enter

ppopcode: Claude에게 요청을 전달합니다.
[sonnet] 로그인 폼을 만들어드릴게요...
         Cursor로 코드를 수정합니다.
```

## 자주 묻는 질문

**Q: Cursor가 꼭 있어야 하나요?**
A: 아니요! 코드 수정 기능은 Cursor가 있으면 자동으로 연동되고, 없으면 그냥 답변만 받을 수 있어요.

**Q: Claude 구독이 필요한가요?**
A: 네, Claude Code 구독이 필요해요. `claude login`으로 로그인해야 합니다.

**Q: 무료인가요?**
A: ppopcode 자체는 무료예요! 다만 Claude Code 구독이 필요합니다.

## 문제가 생겼을 때

### "go를 찾을 수 없어요" 에러
→ Go 설치 후 터미널을 껐다 켜세요.

### 빌드할 때 에러가 나요
→ `go mod tidy`를 먼저 실행해보세요.

### 실행은 되는데 화면이 깨져요
→ 터미널 크기를 키워보세요. 최소 80x24 정도 필요해요.

## 버전

**v1.0.0** - 첫 안정 릴리즈

## 기여하기

버그를 발견하거나 아이디어가 있으면 Issue를 열어주세요!

## 라이선스

MIT - 마음대로 쓰세요!
